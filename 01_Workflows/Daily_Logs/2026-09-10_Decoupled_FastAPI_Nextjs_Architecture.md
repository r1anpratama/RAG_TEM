...
1. **Backend Decoupling (`website/backend/`)**:
   ...
   - Isolated all credentials in `website/backend/.env` with strict `website/backend/.env.example` templates.
   - Built sliding-window rate limiting middleware per client IP to safeguard against bot flooding (HTTP 429).
   - Created `website/backend/tests/test_backend_api.py` with 100% pass rate (6/6 tests).

2. **Frontend Decoupling (`website/frontend/`)**:
...
3. **Repository Consolidation & Clean Directory Structure**:
   - Consolidated all web application modules into `website/`: `website/frontend/`, `website/backend/`, and `website/classic/`.
   - Relocated literature scratch research into `data/literature_search/` and purged `scratch/`.
   - Created unified `website/README.md` guide.

4. **Obsidian Second Brain Documentation**:
...
- Backend tests: `pytest website/backend/tests/` -> 6 passed.
- Core pipeline tests: `pytest tests/` -> 20 passed.
- Total passing tests: 26 (100% green).
