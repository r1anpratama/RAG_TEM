...
function DashboardContent() {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-slate_obsidian-900 text-slate-100 font-sans">
      {/* Top Header */}
      <header className="sticky top-0 z-50 flex h-16 w-full items-center justify-between border-b border-slate-800/80 bg-slate_obsidian-900/90 px-6 backdrop-blur-xl">
        <div className="flex items-center space-x-4">
          <Link
            href="/"
            className="flex items-center space-x-1.5 rounded-lg border border-slate-800 bg-slate_obsidian-card px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-800 hover:text-white transition"
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">E-DREaM Portal</span>
          </Link>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="hidden md:inline-block rounded bg-cyan-500/15 px-2 py-0.5 text-[10px] font-bold text-cyan-400 border border-cyan-500/30 font-mono">
              NCU × NVAITC
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          {/* Scenario Selector */}
          <div className="hidden sm:flex items-center space-x-2 bg-slate-900/80 border border-slate-800 rounded-lg p-1">
            <span className="text-[11px] text-slate-400 pl-2 font-medium">Scenario:</span>
            <select
              ...
              className="bg-slate_obsidian-card text-xs text-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-cyan-400 border border-slate-700/60 cursor-pointer"
            >
              {scenarios.map((sc) => (
          ...
          <div className="flex items-center space-x-1.5 rounded-lg border border-slate-800 bg-slate_obsidian-card px-2.5 py-1 text-[11px] font-mono text-slate-400">
            <span className={`h-2 w-2 rounded-full ${backendHealth ? "bg-emerald-400 animate-pulse" : "bg-rose-500"}`}></span>
      ...
      <div className="sticky top-16 z-40 flex w-full border-b border-slate-800 bg-slate_obsidian-card/95 px-6 backdrop-blur-md overflow-x-auto">
...
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="flex h-screen w-screen items-center justify-center bg-slate_obsidian-900 text-cyan-400 text-xs font-mono">Loading RAG TEM Dashboard...</div>}>
      <DashboardContent />
    </Suspense>
  );
}
