    ...
    <div className="flex flex-col space-y-2 rounded-xl border border-stormy_teal-400/30 bg-ink_black-400/80 p-3 backdrop-blur-sm shadow-md">
      <div className="flex items-center justify-between border-b border-stormy_teal-400/30 pb-1.5">
        <div className="flex items-center space-x-2">
          <GitFork className="h-4 w-4 text-vivid_tangerine-500" />
          <h3 className="text-xs font-bold uppercase tracking-wider text-papaya_whip-500">
            Geo-GraphRAG Cascading Ruptures (TEM Table 2)
          </h3>
        </div>
        <span className="text-[10px] font-mono text-stormy_teal-800 bg-stormy_teal-500/20 px-2 py-0.5 rounded border border-stormy_teal-400/30">
      ...
      <div className="space-y-2">
        {items.map((c, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between rounded-lg border border-stormy_teal-400/30 bg-ink_black-500/90 p-2 text-xs"
          >
            <div className="space-y-0.5">
              <div className="font-bold text-papaya_whip-500">
                {c.pairing_label || `${primaryFaultName} ↔ Fault #${c.target_fault_id}`}
              </div>
              <div className="text-[10px] text-stormy_teal-800">
                Mechanism: <span className="text-papaya_whip-700">{c.rupture_interaction_type || "COSEISMIC_TRANSFER"}</span>
              </div>
            </div>
            <div className="text-right">
              <span className="font-mono font-extrabold text-vivid_tangerine-500">
                Mw {c.combined_mw || c.joint_magnitude_mw || 7.15}
              </span>
              <div className="text-[9px] text-stormy_teal-800 font-mono">
      ...
      <div className="flex items-center justify-between text-[10px] text-stormy_teal-800 border-t border-stormy_teal-400/30 pt-1 font-mono">
        ...
        <span className="text-stormy_teal-700 font-medium">Multi-Hop Traversal</span>
      </div>
    </div>
  );
};
