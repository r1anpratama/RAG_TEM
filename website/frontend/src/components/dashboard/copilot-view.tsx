...
export const CopilotView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[760px] w-full rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate_obsidian-card overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="flex h-16 items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 bg-white dark:bg-slate_obsidian-card">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="flex items-center space-x-1 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 px-3 py-1.5 text-xs text-slate-400 hover:bg-slate-800 hover:text-white transition"
          >
      ...
      {messages.length === 0 && (
        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800/80 bg-slate-900/40">
          ...
          <div className="flex flex-wrap gap-2">
            {samplePrompts.map((prompt, i) => (
              <button
                ...
                className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate_obsidian-card px-3 py-1.5 text-left text-xs text-slate-300 hover:border-cyan-500/40 hover:bg-slate-800 hover:text-white transition"
              >
                {prompt}
              </button>
            ))}
      ...
      {/* Message Input Box */}
      <form onSubmit={handleSubmit} className="border-t border-slate-200 dark:border-slate-800 p-4 bg-white dark:bg-slate_obsidian-card">
        <div className="flex items-center space-x-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-900/90 px-4 py-2 focus-within:border-cyan-500/50 transition">
          <input
            type="text"
            value={inputVal}
            onChange={(e) => setInputVal(e.target.value)}
            placeholder="Ask anything about Taiwan active faults, TEM PSHA 2025, or NCU triage..."
            className="flex-1 bg-transparent text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-500 focus:outline-none"
